export const ProjectCard = (proyecto, categoryId, index) => {
  ...
  // Lógica de presentación web: Dos ventanas separadas 430x600 (base)
  let imagesHtml = '';
  ...
  // Añadimos clase de categoría para estilos CSS específicos (ej: category-ecommerce)
  const categoryClass = `category-${categoryId.toLowerCase()}`;

  return `
    <article class="bento-card ${interactionClass} ${categoryClass} reveal" style="transition-delay: ${delay}s">
      ${glareHtml}
